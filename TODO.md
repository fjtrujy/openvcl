...
- Replace bounded textual lookahead with the new basic-block list scheduler
  hook in `src/VuSchedulerAnalysis.*`.
- Expand the descriptor-derived dependency graph from preserve-order mode into
  the production ready-set scheduler.
...
- Keep `--cost` and `--cost-json` working on both OpenVCL-generated VSM and
  SCE/reference VSM.
- Add before/after scheduler reports keyed by:
  - `top_weighted_estimated_blocks`;
  - `top_weighted_idle_blocks`;
  - `top_weighted_wait_blocks`.
- Generate side-by-side renderer comparison tables from a script or OpenVCL
  mode instead of hand-maintaining Markdown.
